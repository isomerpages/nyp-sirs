---
title: SGUS Omni Commerce & Digital Marketing
permalink: /services/sgus-programme/ocdm
third_nav_title: Career Services
---
<img src="/images/images-2021/Services_SGUS_Overview.png" style="width:100%;">

<h4 style="text-align:center;">Next intake:</h4>

<center><table style="width:80%;">
    <tr style="text-align:center;">
      <td style="text-align:center;width:50%;">7 Feb 2022</td>
    </tr>
</table></center>

<p>
SGUS Programme will end in March 2022. Hurry and sign up now! 
<br>The SGUS Programme in Omni Commerce & Digital Marketing aims to enhance and equip trainees with skillsets and techniques for omni commerce. As digital becomes increasingly part of consumer lifestyles, their changing buying behaviour, consumption habits and expectations become a challenge for businesses to ensure that their online and offline business channels provide a unified experience for their customers. The ability to deploy the right techniques to heighten customer experience and satisfaction will turn opportunities into sales for the business.</p>

<h4>Outline</h4>
<ul>
  <li>Global Mindset (Intermediate) (24 hours)</li>
  <li>Retailing and the Economy (SF Manager) (24 hours)</li>
  <li>Sales Target Management (SF Manager) (24 hours)</li>
  <li>Service Leadership (SF Manager) (30 hours)</li>
  <li>Workplace Communications (SF Manager) (18 hours)</li>
  <li>Customer Experience Innovation (SF Manager) (18 hours)</li>
  <li>Productivity Improvement (SF Manager) (30 hours)</li>
  <li>Financial Analysis (SF Manager) (30 hours)</li>
  <li>Service Leadership (SF – Operations Manager) (24 hours)</li>
  <li>Service Excellence (SF – Operations Manager) (24 hours)</li>
  <li>Omni Commerce Retail (24 hours)</li>
  <li>Omni Commerce Visual (16 hours)</li>
  <li>Getting Started on Qoo10 (Onboarding) (8 hours)</li>
  <li>Driving Sales Performance on Qoo10 (Marketing) (8 hours)</li>
  <li>Getting Started on Lazada (Onboarding) (8 hours)</li>
  <li>Drive Sales Performance on Lazada (Marketing) (8 hours)</li>
  <li>Getting Started on Shopee (Onboarding) (8 hours)</li>
  <li>Driving Sales Performance on Shopee (Marketing) (8 hours)</li>
  <li>Search Engine Optimization – SEO (SF – Manager) (24 hours)</li>
	<li>Paid Search Engine Marketing – SEM (SF – Manager) (24 hours)</li>
  </ul>
  
<h4>Who Should Attend</h4>
<p>Suitable for participants with at least 3 years senior managerial working experience in retail or Professional, Managers, Executives and Technicians (PMETs) position in non-retail sector.</p>
 
<h4>Admission Requirements</h4>
<p>GCE 'O' level Credit (at least C6) in English and Mathematics or Workplace Literacy and Numeracy (WPLN) score: Level 7 for Reading, Listening, Writing, and Numeracy.</p>

<h4>Fees</h4>

<center>
<table style="width:100%;">
<tr>
<th style="width:70%;">Category</th>
<th style="width:30%:">Price</th>
</tr>

<tr>
<td>Nett (including GST)</td>
<td>$500.00</td>
</tr>

</table>
</center>

<p>This programme is also eligible for SkillsFuture Credit. For SkillsFuture course reference, please click <a href="/files/documents-2021/SIRS-SkillsFuture-CourseRefNumber.pdf">here.</a></p>

<div style="width:50%;float:left;"><center><a href="https://form.gov.sg/#!/5f7c08ce956eff0011c11fcf" style="background-color:#06225e; border:white; color:white; padding: 10px 10px; text-align:center; display:inline-block; margin: 4px 2px; cursor:pointer;text-decoration:none;">Register Now</a></center></div>

<div style="width:50%;float:left;"><center><a href="https://form.gov.sg/#!/61adb3bafb55fa00122427cd" style="background-color:#06225e; border:white; color:white; padding: 10px 10px; text-align:center; display:inline-block; margin: 4px 2px; cursor:pointer;text-decoration:none;">Register for Interest</a></center></div>